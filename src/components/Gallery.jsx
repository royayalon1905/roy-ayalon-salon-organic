...
const { gallery: galleryContent } = siteConfig.content
      ...
      <div className="mx-auto max-w-7xl px-6 md:px-10">
        ...
        <div className="mt-12 columns-2 gap-4 sm:columns-3 lg:columns-4">
          {gallery.map((g, i) => (
            <figure
              key={g.id}
              className={`relative mb-4 break-inside-avoid overflow-hidden rounded-lg border border-ink/10 ${ASPECTS[i % ASPECTS.length]}`}
            >
              <img
                src={g.image}
                alt={g.caption}
